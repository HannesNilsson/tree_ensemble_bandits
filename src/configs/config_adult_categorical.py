...
def get_config():
    ...
    #convert and extract feature types
    feature_types = ['c'] + ['q'] + ['c'] + ['q'] + ['c'] + ['q'] + ['c' for i in range(5)] + ['q' for i in range(3)] + ['c']
    ...
    def env_arm_context_and_reward_update_function(arm, advance_data, rng=np.random.default_rng()):
        ...
        context = np.concatenate((label, context.T), axis=0)  #put arm ID first
